...
const RF_ANNUAL = 0.018; // risk-free rate used as the floor for "your dollar" (same reference as the presentation)

// "MM/YYYY" (format of the Client.since field) -> unix seconds for day 1 of that month.
...
// Client's weighted USD return (only products with a registered return) + real coverage
// (how much of the total portfolio value is actually covered by this return, vs. products
// without a registered return yet) — to make clear the line is real data, not 100% estimated.
...
// 3 series in dollars, base $10,000: Client (real: only exists from their entry
// date — Client.since — before that it's "not invested" and the line disappears/turns
// gray on the front end), S&P 500 (real, backtest), and CORE22+/HPC22 (real, backtest).
// Never fabricates — when the client has no estimated return (no detailed portfolio),
// returns only S&P/CORE22+.
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Return/drawdown in percentage points (e.g. 162.5, not 1.625) — same convention as
    // /api/fund-benchmarks, which the front end (pct() helper) already expects.
    ...
    if (blended && client) {
      ...
      // Real path (green) always calibrated from the client's ACTUAL entry date
      // (not the arbitrary start of the chosen period) — preserves the real shape of
      // the crises lived through since they actually started investing.
      ...
      if (investedIdxInWindow > 0 && investedIdxInWindow < tSeries.length) {
        // Client entered INSIDE the displayed window: gray (not invested) before, green after.
        ...
        // Client was already invested before the window started: no gray, just the
        // portion of the green path that falls inside the displayed window.
        const offset = startIdx - greenBaseIdx;
        clientAfter = fullGreenPath.slice(offset).map((v, i) => ({ time: tSeries[i], value: +v.toFixed(0) }));
      } else {
        // Entry falls after the end of the window (shouldn't happen, since is always in the past).
